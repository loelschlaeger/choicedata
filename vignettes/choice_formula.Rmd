...
```r
choice_formula(formula, error_term, random_effects = NULL)
```

Note that only the arguments `formula` and `error_term` are required. The argument `random_effects` per default indicates no random effects.
...
Different distributional assumptions about the error term $\varepsilon$ lead to different choice models. Currently available options are:
...
Optionally, covariates can have random effects, meaning their coefficients can follow a random distribution (known as a mixing distribution). Currently available options are:
