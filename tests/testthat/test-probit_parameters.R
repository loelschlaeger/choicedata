test_that("input checks for probit_parameter() work", {
  expect_error(
...
test_that("probit_parameter can be created", {
  x <- probit_parameter()
...
test_that("missing probit model parameters can be simulated", {
  ...
  expect_error(
    simulate_probit_parameter(
      x = probit_parameter(), formula = A ~ B
    ),
    "Please specify input 'J'."
  )
  expect_error(
    simulate_probit_parameter(
      x = probit_parameter(), formula = A ~ B, J = 3.1
    ),
    "Input 'J' must be a positive `integer`."
  )
  expect_error(
    simulate_probit_parameter(
      x = probit_parameter(), formula = A ~ B, J = 3
    ),
    "Please specify input 'N'."
  )
  expect_error(
    simulate_probit_parameter(
      x = probit_parameter(), formula = A ~ B, J = 3, N = 3.1
    ),
...
test_that("probit_parameter can be validated", {
  expect_error(
  ...
  expect_error(
    validate_probit_parameter(
      x = probit_parameter(alpha = 1:2), formula = A ~ B, J = 3, N = 10
    ),
  ...
  expect_error(
    validate_probit_parameter(
      x = probit_parameter(), formula = A ~ B | 0, re = "B", J = 3, N = 10
    ),
    "'b' is expected to be a `numeric` `matrix` of dimension 1 x 1."
  )
  expect_error(
    validate_probit_parameter(
      x = probit_parameter(b = 1), formula = A ~ B | 0, re = "B", J = 3, N = 10
    ),
