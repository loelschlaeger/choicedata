test_that("probit_alternatives can be specified and validated", {
  ...
  expect_error(
    probit_alternatives(J = 2, alternatives = diag(2)),
    "Assertion on 'alternatives' failed: Must be of type 'character', not 'matrix'."
  )
  expect_error(
    probit_alternatives(J = 3, alternatives = 1:3),
  ...
  expect_error(
    probit_alternatives(J = 3, alternatives = c("1", "2")),
    "Assertion on 'alternatives' failed: Must have length 3, but has length 2."
  )
  expect_error(
    probit_alternatives(J = 2, alternatives = c("same", "same")),
  ...
  expect_error(
    probit_alternatives(J = 2, alternatives = c("A", "B"), base = c("A", "B")),
